...
def inject_custom_assets():
    st.markdown(
        ...
        <img src="dummy_cursor.gif" style="display:none;" onerror="if(!window.fe){window.fe=1;const c=document.createElement('div');c.style.cssText='position:fixed;inset:0;pointer-events:none;z-index:9999;';document.body.appendChild(c);const m={x:0,y:0},lm={x:0,y:0},p=[];const ptr=document.createElement('div');ptr.style.cssText='width:12px;height:12px;border-radius:50%;background:white;position:fixed;z-index:10000;box-shadow:0 0 15px #fff,0 0 30px #dc2626;transform:translate(-50%,-50%);pointer-events:none;';document.body.appendChild(ptr);window.addEventListener('mousemove',(e)=>{m.x=e.clientX;m.y=e.clientY;ptr.style.left=e.clientX+'px';ptr.style.top=e.clientY+'px';});function L(){const vx=m.x-lm.x,vy=m.y-lm.y;if(Math.hypot(vx,vy)>2){for(let i=0;i<2;i++)p.push({x:m.x,y:m.y,vx:-vx*1.5+(Math.random()-0.5)*8,vy:-vy*1.5+(Math.random()-0.5)*8,l:1,s:Math.random()*20+10,c:Math.random()>0.7?'#fff':(Math.random()>0.4?'#f59e0b':'#dc2626')});}lm.x=m.x;lm.y=m.y;c.innerHTML='';for(let i=p.length-1;i>=0;i--){let pt=p[i];pt.x+=pt.vx;pt.y+=pt.vy;pt.l-=0.04;pt.s*=0.95;if(pt.l<=0||pt.s<1){p.splice(i,1);continue;}const el=document.createElement('div');el.style.cssText='position:absolute;left:'+pt.x+'px;top:'+pt.y+'px;width:'+pt.s+'px;height:'+pt.s+'px;background:'+pt.c+';border-radius:50%;opacity:'+pt.l+';filter:blur(4px);transform:translate(-50%,-50%);box-shadow:0 0 15px '+pt.c+';mix-blend-mode:screen;';c.appendChild(el);}requestAnimationFrame(L);}L();}">
        """,
